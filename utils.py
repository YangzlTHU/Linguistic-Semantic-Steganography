...
class Vocabulary(object):
	def __init__(self, data_path, max_len=200, min_len=5, word_drop=5, encoding='utf8', split_token='\n'):
	...
	def _build_vocabulary(self):
		...
		for data_path in self._data_path:
			with open(data_path, 'r', encoding=self._encoding) as f:
				if self._split_token == '\n':
					sentences = f.readlines()
				else:
					sentences = f.read().split(self._split_token)
					sentences.remove('')
...
class Corpus(object):
	def __init__(self, data_path, vocabulary, max_len=200, min_len=5, split_token='\n'):
		if type(data_path) == str:
			data_path = [data_path]
		self._data_path = data_path
...
def split_corpus(data_path, train_path, test_path, max_len=200, min_len=5, ratio=0.8, seed=0, encoding='utf8', split_token='\n'):
	with open(data_path, 'r', encoding=encoding) as f:
		if split_token == '\n':
			sentences = f.readlines()
		else:
			sentences = f.read().split(split_token)
			sentences.remove('')
	sentences = [_ for _ in filter(lambda x: x not in [None, ''], sentences)
	             if len(_.split()) <= max_len and len(_.split()) >= min_len]
	if split_token != '\n':
		sentences = [_ + split_token for _ in sentences]
	np.random.seed(seed)
...
class Generator(object):
	def __init__(self, data, labels, vocabulary=None):
